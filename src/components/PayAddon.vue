<template>
    <a-modal v-model:visible="visible" :title="null" :centered="true" :footer="null" style="height:unset!important;width:400px"  :bodyStyle="{padding:0,height:'605px'}"
      :closable="false" :maskClosable="false" ><iframe v-if="ready" 
            sandbox="allow-forms allow-scripts allow-same-origin" :src="url" title="Pay"
            style="width:100%; height:100%; border:0px; overflow:hidden;"></iframe></a-modal>
</template>
...
const info: any = ref({
    ...
    name: "User: "+props.record?.borrowId+" Book: "+props.record?.name,
    money: props.record?.fine,
    sitename: "Library",
    sign: '',
    sign_type: 'MD5',
})
